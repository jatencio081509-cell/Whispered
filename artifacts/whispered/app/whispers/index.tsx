...
export default function WhispersScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem("whispers").then((d) => {
      if (d) setWhispers(JSON.parse(d));
    });
  }, []);
  ...
  const revealWhisper = (id: string) => {
    ...
    const updated = whispers.map((w) =>
      w.id === id
        ? { ...w, revealed: true, revealedAt: new Date().toISOString() }
        : w,
    );
    save(updated);
  };
  ...
  const deleteWhisper = (id: string) => {
    save(whispers.filter((w) => w.id !== id));
  };

  const getConditionInfo = (key: RevealCondition) =>
            ...
            >
              <View style={styles.whisperHeader}>
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.condLabel, { color: colors.accent }]}
                  >
                    {cond.label}
                  </Text>
                </View>
              ...
              {w.revealed ? (
                <>
                  <Text
                    style={[styles.whisperContent, { color: colors.text }]}
                  >
                    {w.content}
                  </Text>
      ...
      >
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <Text
              style={[styles.condTitle, { color: colors.mutedForeground }]}
            >
              Reveal when
            </Text>
                    ...
                    styles.condBtn,
                    {
                      backgroundColor:
                        condition === c.key
                          ...
                          : colors.surface,
                      borderColor:
                        condition === c.key ? colors.primary : colors.border,
                    },
                  ]}
...
const styles = StyleSheet.create({
  ...
  heroBanner: { flexDirection: "row", alignItems: "center", gap: 14, padding: 20, borderRadius: 18, borderWidth: 1, marginBottom: 4 },
  ...
  whisperCard: { padding: 16, borderRadius: 16, borderWidth: 1, gap: 12 },
  ...
  lockedContent: { flexDirection: "row", alignItems: "center", gap: 10, padding: 14, borderRadius: 10 },
  lockedText: { fontSize: 13, fontFamily: "Inter_400Regular" },
  previewBtn: { paddingVertical: 8, paddingHorizontal: 14, borderRadius: 8, alignSelf: "flex-start" },
  ...
  condTitle: { fontSize: 12, fontFamily: "Inter_500Medium", textTransform: "uppercase", letterSpacing: 0.3 },
  condGrid: { gap: 8 },
